...
AntiBullyStudy2016ActionClient::AntiBullyStudy2016ActionClient(){
...
void AntiBullyStudy2016ActionClient::runActionClient()//int argc, char **argv)
{
...
//void AntiBullyStudy2016ActionClient::runActionClient(ros::NodeHandle nh)//int argc, char **argv)
void AntiBullyStudy2016ActionClient::runActionClientDev()//int argc, char **argv)
{
	ros::NodeHandle nh;
	...
	ros::Rate r(rate);
  	...
  	ros::Subscriber sub_msg = nh.subscribe("/perceptual_filter/behavior", 1000, &ReceiveMsg::messageCallbackContextIndependentBehavior, behavior_msg_wrapper);//&AntiBullyStudy2016ActionClient::messageCallbackContextIndependentBehavior, behavior_msg_wrapper);
	...
	while (ros::ok())
	{
		...
		{		
			...
			for (size_t person_idx = 0; person_idx < audience_behaviors.size(); ++person_idx)
			{
				...
				hitting = individual_behavior.inappropriate_behavior.hitting;
				punching = individual_behavior.inappropriate_behavior.punching;
				kicking = individual_behavior.inappropriate_behavior.kicking;
				shoving = individual_behavior.inappropriate_behavior.shoving;
				...
				if (hitting_prep || hitting) countOccurances["hitting_prep"]++;
				else countOccurances["hitting_prep"] = 0;
				if (punching_prep || punching) countOccurances["punching_prep"]++;
				else countOccurances["punching_prep"] = 0;
				if (kicking_prep || kicking) countOccurances["kicking_prep"]++;
				else countOccurances["kicking_prep"] = 0;
				if (shoving_prep || shoving) countOccurances["shoving_prep"]++;
				...
				// if (hitting) countOccurances["hitting"]++;
				// else countOccurances["hitting"] = 0;
				// if (kicking) countOccurances["kicking"]++;
				// else countOccurances["kicking"] = 0;
				// if (punching) countOccurances["punching"]++;
				// else countOccurances["punching"] = 0;
				// if (shoving) countOccurances["shoving"]++;
				// else countOccurances["shoving"] = 0;
			...
			if (audience_behaviors.size() == 1) //there is only one person tracked
			{
				...
				if (countOccurances["pointing_taunt"] >= 3)
				{
				...
				else if (countOccurances["hitting_prep"] >= 3)
				{
				...
				else if (countOccurances["punching_prep"] >= 3)
				{
				...
				else if (countOccurances["kicking_prep"] >= 3)
				{
				...
				else if (countOccurances["shoving_prep"] >= 3)
				{
				...
				else if (countOccurances["pointing"] >= 3)
				{
