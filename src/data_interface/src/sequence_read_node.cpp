...
vector<string> FilenamesWithoutPattern(vector<string> strings, string pattern)
{
    auto pos = remove_if(begin(strings), end(strings), 
                              [&](string& s) { return s.find(pattern) != string::npos ; }) ; 
...
void ReadPersonDataFromDirectory(vector<vector<float> >& data, map<string, int>& headermap, string directorypath, vector<string> directoryfilenames, string datasource)
{ 
...
_KinectBodiesMsg GetKinectBodyMsg(vector<float> featureVectorKinectBodies, map<string, int> headerMapKinectBodies)
{
	...
	for(int i = 0; i < 25; ++i)
	{
		...
		zp = featureVectorKinectBodies[headerMapKinectBodies[jointname + "_position_z"]] ;
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "sequence_read_node");
  ros::NodeHandle nh;
  ...
  for (string folderName : filenamesWithPattern)
  {
  	  ...
  	  vector<string> filenames2 = GetFileNames(directoryPath);

  	  vector<string> gesture_names = {"pointing", "punching", "hitting", "showing fist", "kicking", "null"};

  	  for (string gesture_name : gesture_names)
  	  {

 	  	  vector<string> filenamesWithPattern2 = FilenamesContainingPattern(filenames2, gesture_name);

 	  	  for (auto file_name : filenamesWithPattern2)
 	  	  {
 	  	  	cout << file_name << endl;
 	  	  }
			//if "prep" not in the foldername, then exclude them from a list
		  if (gesture_name.find("prep") == std::string::npos) { 
		  	  filenamesWithPattern2 = FilenamesWithoutPattern(filenamesWithPattern2, "prep");
		  }


	 	  for (string folderName2 : filenamesWithPattern2)
	 	  {
	 	  	string directoryPath2 = directoryPath + "/" + folderName2;

			map< string, map<string, int> > headerMap;
			map< string, vector<vector<float> > > allData;
		  	vector<string> directoryFilenames = GetFileNames(directoryPath2);

		  	if (processCLMHeads) ReadPersonDataFromDirectory(allData["clmHeads"], headerMap["clmHeads"], directoryPath2, directoryFilenames, "CLMHeads");//"/home/TrainingData/Test/logCLMHeads_4-14-2016_10-41-51_person1.csv");
		  	if (processKinectBodies) ReadPersonDataFromDirectory(allData["kinectBodies"], headerMap["kinectBodies"], directoryPath2, directoryFilenames, "KinectBodies");
		  	if (processKinectFaces) ReadPersonDataFromDirectory(allData["kinectFaces"], headerMap["kinectFaces"], directoryPath2, directoryFilenames, "KinectFaces");
		  	if (processKinectHDFaces) ReadPersonDataFromDirectory(allData["kinectHDFaces"], headerMap["kinectHDFaces"], directoryPath2, directoryFilenames, "KinectHDFaces");
		  	if (processRawAudio) ReadDataFromDirectory(allData["kinectRawAudio"], headerMap["kinectRawAudio"], directoryPath2, directoryFilenames, "KinectRawAudio");

	 	  	//cout << "size of kinectBodies " << directoryPath2 << endl;
			//directoryPath2
	 	  	//cout << "size of kinectBodies " << allData["kinectBodies"].size() << endl;


	 	  	int curTime;
	 	  	int dataIndex = 0;

	 	  	while (nh.ok() && dataIndex != allData["kinectBodies"].size() )
	 	  	{

	 	  		curTime = allData["kinectBodies"][dataIndex][0]; //assume I must be doing body tracking

	 	  		if (processCLMHeads) assert(curTime == allData["clmHeads"][dataIndex][0]);
	 	  		if (processKinectFaces) assert(curTime == allData["kinectFaces"][dataIndex][0]);
	 	  		if (processKinectHDFaces) assert(curTime == allData["kinectHDFaces"][dataIndex][0]);
	 	  		if (processRawAudio) assert(curTime == allData["kinectRawAudio"][dataIndex][0]); // DOUBLE CHECK THIS ASSERTION, assumes 30 fps max

				multimodal::MultimodalState multimodal_msg; 

				//load feature vectors in multimodal msg
				for (auto const& element : allData)
				{
					string key = element.first;
					vector<float> features = allData[key][dataIndex];
				...
				//load label in multiodal msg
				auto & ros_labels_msg = multimodal_msg.labels_msg;

				multimodal_msg.sequence_number = sequenceNum;
				if (dataIndex == 0)
				{
					multimodal_msg.sequence_boundary = "start";
				}
				else if (dataIndex == allData["kinectBodies"].size() - 1)
				{
					multimodal_msg.sequence_boundary = "end";
				}
				else
				{
					multimodal_msg.sequence_boundary = "";
				}
				...
				ros_label_msg.labelName = gesture_name;
				ros_label_msg.labelValue = 1;
				ros_label_msg.isValid = true;
				...
				pub_message.publish(multimodal_msg);
	 	  		...
	 	  		//cout << "sequence number " << multimodal_msg.sequence_number << ", boundary " << multimodal_msg.sequence_boundary << endl;
