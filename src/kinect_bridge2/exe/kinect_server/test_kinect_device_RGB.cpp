...
int main()
{
    ...
    while( true )
    {
        try
        {
			...
			uint16_t const crop_x = 0;// message_ptr->header_.width_;
			uint16_t const crop_y = 0;// message_ptr->header_.height_;
			uint16_t const crop_w = message_ptr->header_.width_;// -(2.0f * crop_x);
			uint16_t const crop_h = message_ptr->header_.height_;// -(crop_y + 0.8f*crop_y);
        ...
        {
            std::cout << e.what() << std::endl;
			std::cout << "kinect_device.pullColorImage failed, trying again" << std::endl;
