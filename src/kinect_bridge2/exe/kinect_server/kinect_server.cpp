...
//                uint16_t const crop_h = message_ptr->header_.height_ - ( crop_y + 0.8f*crop_y );
				...
				uint16_t const crop_x = 0;
				uint16_t const crop_y = 0;
				uint16_t const crop_w = message_ptr->header_.width_;
				uint16_t const crop_h = message_ptr->header_.height_;
...
public:
    ...
    void run()
    {
        while( running_ )
        {
			//std::cout << "running read speech task" << std::endl;
            // if the output fifo is too full, wait for consumers to pop items off
            {
                auto output_handle = output_fifo_.getHandle();
...
int main( int argc, char ** argv )
{
    ...
    while( true )
    {
        try
        {
			...
			kinect_device.initialize( true, false, false, true, true, true );
            break;
        }
        catch( KinectException & e )
        {
